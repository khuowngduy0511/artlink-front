...
import { ArtlinkDetailType, AssetType, CommentType } from "./ArtlinkDetailType";
...
function castResponseData(data: any): ArtlinkDetailType {
  return {
    id: data.id,
    account: {
      id: data.account.id,
 ...
 * @returns {Promise<ArtlinkDetailType>}
 * @example
 ...
 * @version 2.2.2
 */
export async function fetchArtlinkDetail(
  id: string | undefined,
  accountId?: string
): Promise<ArtlinkDetailType> {
  try {
    if (accountId) {
 ...
 * @returns () => void
 * @example
 * const unsubscribe = fetchCommentsForArtlinkRealTime("artlinkId123", setComments);
 * unsubscribe();
 * @version 1.0.0
 * @author ThongNT
 */
export function fetchCommentsForArtlinkRealTime(
  artworkId: string,
 ...
 * @version 1.1.0
 */
export async function editCommentOnArtwork(commentId: string, newText: string): Promise<any> {
  try {
    const response = await axiosPrivate.put(`${BASE_URL}/comments/${commentId}`, {
      commentText: newText,
    });
    return response;
  } catch (error) {
