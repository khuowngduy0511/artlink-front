...
export const CreateAProposal = (
  ...
  console.log("[CreateProposal] Form values:", values);
  console.log("[CreateProposal] totalPrice:", values.totalPrice);
  console.log("[CreateProposal] depositPercent:", values.depositPercent);
  console.log("[CreateProposal] calculated initialPrice:", (values.totalPrice || 0) * (values.depositPercent || 0));
  ...
  const formattedData: ProposalFormType = {
    ...
    initialPrice: (values.totalPrice || 0) * (values.depositPercent || 0),
    total: values.totalPrice || 0,
  };

  console.log("[CreateProposal] Sending to API:", formattedData);
