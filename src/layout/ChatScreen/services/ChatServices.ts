...
export function GetMessagesByChatboxIdRealTime(
  ...
  const url = `${WS_URL}/api/chatbox/${chatboxId}/messages/ws`;
  const socket = new WebSocket(url);
  ...
  socket.onmessage = (event) => {
    ...
    const areEqual = arraysChatMsgEqual(_tmpMessages, message);

    if (Array.isArray(message) && !areEqual) {
      // console.log("[WebSocket] ✅ New messages detected, updating UI");
