...
export async function CreateServiceData(formValue: any): Promise<any> {
  try {
    // Validate thumbnail trước khi tạo FormData
    if (!formValue.thumbnail || !(formValue.thumbnail instanceof File)) {
      throw new Error("Vui lòng chọn ảnh đại diện cho dịch vụ");
    }
    ...
    // Đảm bảo referenceArtworks là một array
    const artworkReferences = Array.isArray(formValue.referenceArtworks) 
      ? formValue.referenceArtworks 
      : [];
    
    // Chỉ append các artwork ID hợp lệ (không rỗng)
    artworkReferences
      .filter((id: string) => id && id.trim() !== "")
      .forEach((id: string) => formData.append("ArtworkReference", id));
