...
import { GetArtlinksData } from "../../ArtlinksView/ArtlinksService";
...
const ServiceInformationSection: React.FC<ServiceInformationProps> = ({
  ...
  const [artworks, setArtworks] = useState<ArtlinkProps[]>(props?.artworkReferences || []);
  ...
  useEffect(() => {
    const handleGetArtworks = async () => {
      try {
        const response = await GetArtlinksData(4, pageNumber, accountId, 1, 0);
