    ...
    return response.data.map((item: any) => {
      return {
        ...
        withdrawalRequest: item.withdrawalRequest
          ? {
              id: item.withdrawalRequest.id,
              amount: item.withdrawalRequest.amount,
              bankCode: item.withdrawalRequest.bankCode,
              bankName: item.withdrawalRequest.bankName,
              bankAccountNumber: item.withdrawalRequest.bankAccountNumber,
              bankAccountName: item.withdrawalRequest.bankAccountName,
              status: item.withdrawalRequest.status,
              adminNote: item.withdrawalRequest.adminNote,
              walletBalanceSnapshot: item.withdrawalRequest.walletBalanceSnapshot,
              createdOn: formatTime(item.withdrawalRequest.createdOn),
              processedOn: item.withdrawalRequest.processedOn ? formatTime(item.withdrawalRequest.processedOn) : null,
            }
          : null,
...
/**
 * Deposits coins into the user's wallet using PayOS.
 *
 * @param amount - The amount of coins to deposit.
 * @returns Payment URL from PayOS
 * @param AnhDH
 * @version 2.0.0
 */
export async function DepositCoins(amount: number) {
  try {
    const response: any = await axiosPrivate.post(`${API_URL}/payos/deposit`, {
      amount: amount
    });
 ...
 */
/**
 * Gets list of available banks for bank account selection
...
export async function GetBankList() {
  try {
    const response = await axiosPrivate.get(`${API_URL}/payments/bank-list`);
    if (response.status !== 200) {
      return [];
    }
    return response.data;
...
/**
 * Updates the wallet bank account information
 ...
 * @version 2.0.0
 */
export async function UpdateBankAccount(bankAccountNumber: string, bankCode: string, bankName: string) {
  try {
    const response = await axiosPrivate.put(`${API_URL}/payments/update-bank-account`, {
      bankAccountNumber,
      bankCode,
      bankName,
    });
 ...
 * @param amount - The amount to withdraw (minimum 10,000 Xu)
 * @returns A Promise that resolves to the withdrawal request data
 * @author AnhDH
 * @version 3.0.0
 */
export async function CreateWithdrawalRequest(amount: number) {
  try {
    const response = await axiosPrivate.post(`${API_URL}/payments/withdrawal-request`, {
      amount,
    });
    if (response.status !== 200) {
    ...
    throw new Error(error.response?.data?.Message || "Không thể tạo yêu cầu rút tiền. Vui lòng kiểm tra lại thông tin tài khoản ngân hàng và số dư.");
  }
}

/**
 * Gets user's withdrawal requests
 * @returns A Promise that resolves to the list of withdrawal requests
 * @author AnhDH
 * @version 3.0.0
 */
export async function GetWithdrawalRequests() {
  try {
    const response = await axiosPrivate.get(`${API_URL}/payments/withdrawal-requests`);
    if (response.status !== 200) {
      return [];
    }
    return response.data.map((item: any) => ({
