...
import { ArtlinkDetailType } from "../ArtlinkDetailScreen/ArtlinkDetailType";
import { fetchArtlinkDetail, fetchIsFollow } from "../ArtlinkDetailScreen/Service";
...
export type awDetailStateToolsType = {
  selectingAw: ArtlinkProps;
  setSelectingAw: React.Dispatch<React.SetStateAction<ArtlinkProps>>;
  isLoading?: boolean;
  currentAwDetail: ArtlinkDetailType;
  setCurrentAwDetail: React.Dispatch<React.SetStateAction<ArtlinkDetailType>>;
  isLiked: boolean;
...
const HomeScreen: React.FC<{ isLogin: boolean }> = ({ isLogin }) => {
  ...
  const [artworks, setArtworks] = useState<ArtlinkProps[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [pageNumber, setPageNumber] = useState(1);
  const [selectingAw, setSelectingAw] = useState<ArtlinkProps>({} as ArtlinkProps); //an artwork from the artworks list state
  const [isLoadingDetail, setIsLoadingDetail] = useState(false);
  const [currentAwDetail, setCurrentAwDetail] = useState<ArtlinkDetailType>({} as ArtlinkDetailType);
  ...
  const fetchDetail = () => {
    ...
    setCurrentAwDetail({} as ArtlinkDetailType);
    fetchArtlinkDetail(selectingAw?.id, currentUserId)
      .then((res) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        let newArtworksData: ArtlinkProps[];
        if (activeTab === 0) {
