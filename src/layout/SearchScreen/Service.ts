 ...
 * @returns Promise<ArtlinkProps[]> list of artworks that match the keyword
 * @example
 * ```
...
export async function searchArtworksByKeyword(
  ...
  const artworks: ArtlinkProps[] = res.data?.items?.map(
    (artwork: any) =>
      ({
        id: artwork?.id,
        title: artwork?.title,
...
function castElt2ArtlinkProps(artworks: any[]): ArtlinkProps[] {
 ...
 *
 * @returns Promise<ArtlinkProps[]> list of artworks that are similar to the interacted artworks
 * @example
 * ```
 ...
 * @author @thongnt0208
 */
export async function GetSimilarAwsByCookie(): Promise<ArtlinkProps[]> {
  ...
  try {
    ...
    return castElt2ArtlinkProps(res.data?.items);
  } catch (error: any) {
